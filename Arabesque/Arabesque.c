...
static void init( const int in_WIDTH, const int in_HEIGHT ) {
    ...
    s_dataRAW = malloc( sizeof(float) * ROWS * COLS );
    s_dataRAW2 = malloc( sizeof(float) * ROWS * COLS );
    s_data = malloc( sizeof(float) * ROWS * COLS );
    s_dataDecay = malloc( sizeof(float) * ROWS * COLS );
    s_points = malloc( sizeof(VertexData_t) * ROWS * COLS * 6 );
    s_points2 = malloc( sizeof(VertexData_t) * ROWS * COLS * 6 );
...
static void update() {
    ...
    GLfloat scaleX = s_screenAspect * 2.0f / (GLfloat)( COLS + 1 );
    ...
    for ( y = 0; y < ROWS; y++ ) {
        ...
        for ( x = 0; x < COLS; x++ ) {
            ...
            GLfloat alpha = ( 1.0f - fabsf( s_data[xy] - 0.5f ) * 2.0f );
